...
| Public release privacy audit | Done | `sentineldesk privacy release-audit --path sentinel-desk` detects local ignored development artifacts (`.agent-venv`, `.demo`, `sentineldesk.egg-info`, and `__pycache__/` directories), while `sentineldesk privacy release-package --source sentinel-desk --output /private/tmp/.../sentinel-desk.release.zip` excludes those artifacts; the latest extracted release ZIP passed `privacy release-audit --require-clean` with 121 scanned files and 0 issues | Use `privacy release-package` rather than direct local-tree zipping for public sharing |
...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` uses Node.js 24-compatible `actions/checkout@v6` and `actions/setup-python@v6`, then runs a Python 3.11 package metadata check, 283 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
| Confirmation-gated bulk task review | Done | `/api/tasks/review/bulk` and `sentineldesk tasks bulk-review` can mark the current filtered task queue as `done`, `reviewed`, `ignored`, or `needs_verification` only after explicit confirmation; confirmation IDs are recorded in local approval records, replay is blocked, each task still writes a local `task.review` audit event, and the calendar assistant exposes bulk buttons for the current kind/status filter | Next practical UI work is local task review history and undo controls |
...
Current expected result: 283 tests pass.
