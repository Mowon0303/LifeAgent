...
| Reliability tests | Done | 288 unittest cases pass in `sentinel-desk/tests`, covering email/calendar extraction, stored-evidence reprocessing, daily landing workflow, dashboard daily summary/run APIs, connector redaction in daily summaries, sync summary redaction, relative deadline windows, expanded date forms, adaptive deadline cap/stuffing handling, non-dollar and spelled-out amounts, high/low-confidence and semantic amount false-positive filters, deadline false-positive filters, action false-positive filters including link/HTML action artifacts, expanded action verbs/noise filters, task review grouping/backend, task evidence drill-down, task queue filters/batch navigation, confirmation-gated bulk task review, local task review history/undo, dashboard APIs, retention/audit/approval gates, privacy/release packaging, RAG safety, model/provider boundaries, LangGraph-shaped workflow metadata, tool-verified latest facts, portal fallback citation chaining, CLI `ask`, Gmail-first integration readiness reports and package shape evals, golden-set eval regression gates, and UI contract shape gates | Re-run before publishing |
| UI contract handoff | Done | `docs/UI_CONTRACT.md` documents response shapes for `/api/calendar/events`, `/api/tasks` with `status`/`kind`/`limit` filters, `/api/tasks/evidence`, `/api/tasks/review`, `/api/tasks/review/bulk`, `/api/tasks/review/history`, `/api/tasks/review/undo`, `/api/daily/summary`, `/api/daily/run`, `/api/calendar/sync`, `/api/calendar/drafts/update`, and `/api/ask`, plus the design-token mapping rules (pending=dashed `approval_state: draft`, confirmed=solid `approved`, source-trust captions, undated handling, task review cards, source-evidence drill-down, task queue filters, cursor navigation, confirmation-gated bulk review buttons, and review history/undo controls); `fixtures/ui/` holds synthetic sample emails and committed sample responses including `daily_summary.sample.json`; `tests/test_ui_contract.py` gates 33 regression assertions including task filters, bulk task review, task review history/undo, task evidence drill-down, daily summary/run, confirm/replay flows, ignore/done review flows, citation payload shape, fixture/live shape sync, and page wiring | Update the contract doc and fixtures together whenever an API shape changes |
...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` uses Node.js 24-compatible `actions/checkout@v6` and `actions/setup-python@v6`, then runs a Python 3.11 package metadata check, 288 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
| Local task review history and undo controls | Done | `/api/tasks/review/history` and `sentineldesk tasks history` list recent local `task.review`/`task.review.bulk` audit events with previous-state metadata and undo availability; `/api/tasks/review/undo` and `sentineldesk tasks undo` require explicit confirmation IDs, restore previous local review state or delete the review row when the task had never been reviewed, write local approval/audit records, block already-undone source audits, and the calendar assistant exposes `查看历史` plus per-row `撤销` controls | Next practical UI work is task priority ranking/default sort controls for high-risk queues |
...
Current expected result: 288 tests pass.
