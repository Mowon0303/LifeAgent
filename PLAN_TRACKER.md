...
| Action lexicon expansion | Done | `extract_email_facts` now recognizes expanded life-admin action verbs (`contact`, `register`, `apply`, `dispute`, `redeem`, `update`, `cancel`, `verify`, `reply`, `bring`, `report`, `check`, `add`, `print`, `enroll`, `contest`) with targeted filters for email-address local parts, noun-like `update/check/report`, `Terms apply`, app-store update prompts, and conditional security-support prompts; targeted tests cover expanded verbs plus known noise cases; golden-set eval improved raw action recall from 0.805 to 1.000 and raw action F1 from 0.838 to 0.939 | Completed; no action false negatives remain in the current golden set |
...
Current expected result: 261 tests pass.
