...
- Next plan to complete: The portfolio engineering loop is closed: eval baseline, UI contract + accepted calendar page, guard-railed model-in-the-loop with cost attribution, stored-evidence ask, and the pivot postmortem are all Done. The next plan is the extraction-improvement queue from `docs/EVAL_REPORT.md` (relative deadlines first, then non-dollar currencies and the action lexicon), raising the eval gate floors deliberately with each fix; record the 2-minute portfolio demo when convenient. Calendar live writes remain deferred unless the product workflow needs confirmed external calendar sync.
...
| Email extraction golden-set eval | Done | `evals/golden/` holds 142 hand-labeled synthetic cases across 10 categories (7 life-admin verticals plus edge_cases, negatives, adversarial) with a written labeling policy in `evals/golden/README.md`; `sentineldesk/evals/email_extract.py` scores field-level precision/recall/F1 on raw and high-confidence layers plus confidence-bucket calibration; `sentineldesk eval email-extract --report-md docs/EVAL_REPORT.md` generates the committed report; measured baseline (2026-06-11): raw deadline P=0.736/R=0.844, raw amount P=0.755/R=0.934, raw action P=0.875/R=0.805, high-confidence deadline P=0.810, high-confidence amount P=0.796, and the risk-word heuristic measurably beats the low bucket (deadline 0.810 vs 0.675); `tests/test_eval_email_extract.py` gates 9 regression assertions including metric floors, calibration direction, suppression-injection resistance, and golden-set integrity | Raise metric floors deliberately when extraction improves; reuse the harness for a redacted real-mailbox distribution report later |
...
- Latest eval milestone: the extraction layer now has a quantitative golden-set baseline (`docs/EVAL_REPORT.md`). Headline findings: the date regex misses all relative deadlines ("within 10 days" style phrases used by lease/legal/subscription mail), the amount regex misses non-dollar currencies, the 14-verb action lexicon misses common life-admin CTAs (contact, register, apply, dispute, redeem, update, cancel, verify), every narrative/marketing/injected date or amount becomes a false positive at the raw layer, and a stuffing attack can push a real deadline past the 10-item extraction cap. These quantified gaps are the work queue for extraction improvements and the motivating evidence for the review-queue product layer.
...
Current expected result: 232 tests pass.
