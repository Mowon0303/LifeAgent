...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` uses Node.js 24-compatible `actions/checkout@v6` and `actions/setup-python@v6`, then runs a Python 3.11 package metadata check, 275 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
- Current landing focus: the calendar assistant now calls the daily-loop APIs so the user sees inbox/stored-mail status, grouped task queue, calendar draft summary, connector readiness, uncertainty, and confirmation-gated actions in one surface.
...
Current expected result: 275 tests pass.
