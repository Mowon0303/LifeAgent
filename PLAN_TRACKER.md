...
- Next plan to complete: The portfolio engineering loop is closed: eval baseline, UI contract + accepted calendar page, guard-railed model-in-the-loop with cost attribution, stored-evidence ask, pivot postmortem, relative-deadline extraction, non-dollar amount extraction, action-lexicon expansion, date-form expansion, spelled-out amount extraction, high-confidence amount false-positive filtering, low-confidence amount false-positive filtering, semantic amount/source-trust filtering, deadline false-positive filtering, extraction-cap/stuffing handling, action false-positive filtering, redacted Gmail-first package shape evals, Gmail-first demo prep, GitHub project packaging, GitHub Actions CI, static portfolio case study, interview project brief, and final UI design reference are all Done. The raw extraction-improvement queue from `docs/EVAL_REPORT.md` is complete. GitHub repository description/topics and portfolio screen recording are intentionally deferred by user decision on 2026-06-12. The next practical plan is product landing: turn the Gmail-first assistant into a repeatable daily workflow with real inbox refresh, task review, calendar draft review, and clear local safety controls. Calendar live writes remain deferred unless the product workflow needs confirmed external calendar sync.
...
| Screen recording approval guard | Done | Recording helper requires interactive `record` confirmation or `SENTINEL_RECORD_APPROVED=1`; `SENTINEL_RECORD_DRY_RUN=1` verified setup without recording, and unapproved non-interactive execution exits before capture; this remains a safety guard even though recording is no longer active scope | Keep the guard for optional future use |
| Product landing loop | Partial | User redirected the project away from portfolio recording on 2026-06-12: the work should prove daily utility rather than demo polish; existing Gmail sync, stored-evidence ask, task review, calendar drafts, retention, redaction, and local dashboard pieces are available | Make the real daily loop explicit: refresh inbox, review extracted tasks, resolve/ignore items, inspect calendar drafts, and keep all writes confirmation-gated |
...
- Current landing focus: make the real daily loop work end to end from Gmail refresh to task review, calendar draft review, uncertainty handling, and confirmation-gated local actions.
