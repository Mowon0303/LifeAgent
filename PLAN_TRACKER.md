...
- Next plan to complete: The portfolio engineering loop is closed: eval baseline, UI contract + accepted calendar page, guard-railed model-in-the-loop with cost attribution, stored-evidence ask, pivot postmortem, relative-deadline extraction, non-dollar amount extraction, action-lexicon expansion, date-form expansion, spelled-out amount extraction, high-confidence amount false-positive filtering, and low-confidence amount false-positive filtering are all Done. The next extraction-improvement queue item from `docs/EVAL_REPORT.md` is semantic amount/source-trust filtering for remaining credit-limit and lookalike-domain phishing fee cases, plus deadline false-positive filtering, followed by extraction-cap/stuffing handling; record the 2-minute portfolio demo when convenient. Calendar live writes remain deferred unless the product workflow needs confirmed external calendar sync.
...
| Email extraction golden-set eval | Done | `evals/golden/` holds 142 hand-labeled synthetic cases across 10 categories (7 life-admin verticals plus edge_cases, negatives, adversarial) with a written labeling policy in `evals/golden/README.md`; `sentineldesk/evals/email_extract.py` scores field-level precision/recall/F1 on raw and high-confidence layers plus confidence-bucket calibration; `sentineldesk eval email-extract --report-md docs/EVAL_REPORT.md` generates the committed report; measured baseline after relative-deadline, date-form, non-dollar/spelled-out amount, high/low-confidence amount false-positive filters, and action-lexicon support (2026-06-11): raw deadline P=0.763/R=0.975, raw amount P=0.974/R=1.000, raw action P=0.885/R=1.000, high-confidence deadline P=0.852/R=0.566, high-confidence amount P=0.977/R=0.553, and the high bucket still beats or matches the low bucket (deadline 0.852 vs 0.667; amount 0.977 vs 0.971); `tests/test_eval_email_extract.py` gates 9 regression assertions including raised deadline/amount/action metric floors, calibration direction, suppression-injection resistance, and golden-set integrity | Next extraction work: semantic amount/source-trust filters plus deadline false-positive filters |
...
Current expected result: 253 tests pass.
