...
| Action false-positive filters | Done | Action extraction now suppresses prompt-injected write/calendar/payment instructions, phishing payment actions, noun-like payment schedules, soft phrases such as `pay no attention`, social-notification sign-in bait, optional survey CTAs, pull-request review requests, and corporate all-hands question forms while preserving real sign-in, schedule, and submit actions; targeted tests cover each trap class and preservation cases; golden-set eval removed all 11 raw action false positives, lifting raw action to P=1.000/R=1.000/F1=1.000 | Move to redacted Gmail-first package shape evals or portfolio demo recording |
...
Current expected result: 263 tests pass.
