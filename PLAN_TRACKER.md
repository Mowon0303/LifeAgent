...
| Reliability tests | Done | 272 unittest cases pass in `sentinel-desk/tests`, covering email/calendar extraction, stored-evidence reprocessing, daily landing workflow, connector redaction in daily summaries, sync summary redaction, relative deadline windows, expanded date forms, adaptive deadline cap/stuffing handling, non-dollar and spelled-out amounts, high/low-confidence and semantic amount false-positive filters, deadline false-positive filters, action false-positive filters including link/HTML action artifacts, expanded action verbs/noise filters, task review grouping/backend, dashboard APIs, retention/audit/approval gates, privacy/release packaging, RAG safety, model/provider boundaries, LangGraph-shaped workflow metadata, tool-verified latest facts, portal fallback citation chaining, CLI `ask`, Gmail-first integration readiness reports and package shape evals, golden-set eval regression gates, and UI contract shape gates | Re-run before publishing |
...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` uses Node.js 24-compatible `actions/checkout@v6` and `actions/setup-python@v6`, then runs a Python 3.11 package metadata check, 272 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
- Current landing focus: the backend daily loop is implemented and live-verified through `sentineldesk daily run`; next UI work should call this flow or equivalent APIs so the user sees inbox refresh status, task queue, calendar draft review, uncertainty, and confirmation-gated actions in one daily surface.
- Latest landing verification: `python3 -B -m sentineldesk --home /private/tmp/lifeagent-daily-smoke daily run --email-json fixtures/ui/sample_emails.json` returned `mode: daily_landing`, 4 messages persisted, 8 facts, 3 local calendar drafts, 8 tasks, redacted connector/cursor handling, and no external writes.
...
Current expected result: 272 tests pass.
