...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` uses Node.js 24-compatible `actions/checkout@v6` and `actions/setup-python@v6`, then runs a Python 3.11 package metadata check, 276 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
- Current landing focus: the calendar assistant now calls the daily-loop APIs and full task review queue so the user sees inbox/stored-mail status, grouped task queue, amount/action task cards, calendar draft summary, connector readiness, uncertainty, and confirmation-gated actions in one surface.
- Latest landing verification: `python3 -B -m sentineldesk --home /private/tmp/lifeagent-daily-smoke daily run --email-json fixtures/ui/sample_emails.json` returned `mode: daily_landing`, 4 messages persisted, 8 facts, 3 local calendar drafts, 8 grouped tasks, redacted connector/cursor handling, and no external writes.
...
Current expected result: 276 tests pass.
