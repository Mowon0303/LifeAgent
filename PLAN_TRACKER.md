...
- Next plan to complete: The portfolio engineering loop is closed: eval baseline, UI contract + accepted calendar page, guard-railed model-in-the-loop with cost attribution, stored-evidence ask, pivot postmortem, relative-deadline extraction, non-dollar amount extraction, action-lexicon expansion, date-form expansion, spelled-out amount extraction, high-confidence amount false-positive filtering, low-confidence amount false-positive filtering, and semantic amount/source-trust filtering are all Done. The next extraction-improvement queue item from `docs/EVAL_REPORT.md` is deadline false-positive filtering, followed by extraction-cap/stuffing handling; record the 2-minute portfolio demo when convenient. Calendar live writes remain deferred unless the product workflow needs confirmed external calendar sync.
...
| Email extraction golden-set eval | Done | `evals/golden/` holds 142 hand-labeled synthetic cases across 10 categories (7 life-admin verticals plus edge_cases, negatives, adversarial) with a written labeling policy in `evals/golden/README.md`; `sentineldesk/evals/email_extract.py` scores field-level precision/recall/F1 on raw and high-confidence layers plus confidence-bucket calibration; `sentineldesk eval email-extract --report-md docs/EVAL_REPORT.md` generates the committed report; measured baseline after relative-deadline, date-form, non-dollar/spelled-out amount, high/low-confidence and semantic amount false-positive filters, and action-lexicon support (2026-06-11): raw deadline P=0.763/R=0.975, raw amount P=1.000/R=1.000, raw action P=0.885/R=1.000, high-confidence deadline P=0.852/R=0.566, high-confidence amount P=1.000/R=0.553, and the high bucket still beats or matches the low bucket (deadline 0.852 vs 0.667; amount 1.000 vs 1.000); `tests/test_eval_email_extract.py` gates 9 regression assertions including raised deadline/amount/action metric floors, calibration direction, suppression-injection resistance, and golden-set integrity | Next extraction work: deadline false-positive filters |
...
| Semantic amount/source-trust filters | Done | Numeric amount extraction now suppresses informational credit-limit increases and lookalike-sender phishing payment requests while preserving real processing fees, housing deposits, and minimum payments; targeted tests cover both remaining amount false-positive traps and preservation cases; golden-set eval reduced raw amount false positives from 2 to 0 and high-confidence amount false positives from 1 to 0, lifting raw amount to P=1.000/R=1.000 and high-confidence amount to P=1.000/R=0.553 | Move to deadline false-positive filters |
...
Current expected result: 255 tests pass.
