...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` runs a Python 3.11 package metadata check, 270 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
- Latest live-run-derived cleanup: a privacy-preserving replay over 54 stored live messages found link/HTML action noise; after the filter, replayed action facts dropped from the previously persisted 732 to 197 while deadline and amount counts stayed at 102 and 218. The committed regression uses synthetic `.example` fixtures only.
...
Current expected result: 270 tests pass.
