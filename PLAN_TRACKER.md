...
- Current landing focus: the backend daily loop is implemented through `sentineldesk daily run`; next UI work should call this flow or equivalent APIs so the user sees inbox refresh status, task queue, calendar draft review, uncertainty, and confirmation-gated actions in one daily surface.
