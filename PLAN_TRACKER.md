...
- Next plan to complete: Gmail-first live verification is the current external-service milestone and is now verified: the user-approved Google OAuth token flow ran, real Gmail readonly sync persisted a non-secret cursor and extracted deadline evidence, `integrations check --suite gmail --require-ready --package` produced a ready redacted package, and redacted-output/source-release privacy audits are clean. Calendar live writes are deferred to the later action-layer milestone; if resumed, use one seeded verification draft and explicit confirmation before any Google/Apple Calendar write.
...
| Reliability tests | Done | 184 unittest cases pass in `sentinel-desk/tests`, including recording-prep CLI coverage, module entrypoint exit-code coverage, Chrome launcher coverage, dashboard smoke, package export, package download route, structured share-package redaction, redacted-output privacy audit, project-tree release audit, clean release-package export, completion-audit privacy/source-release/action-plan coverage, human-readable live handoff checklist coverage, lease/rent coverage, CDP screenshot artifacts, email extraction, persisted email/calendar drafts, calendar event view API, local calendar draft edit/reschedule, dashboard calendar draft update API, dashboard ICS confirmation sync, dashboard retention preview/confirmed purge API, remote calendar dedupe/update before create, CLI Google/Apple calendar sync confirmation gates, durable approval records, calendar safety, assistant routing, evidence-backed alert explanation, status meaning, next-step recommendation, RAG-backed policy answers with citations, source conflicts, cross-source stored conflict detection, connector boundaries, connector cursors, secret redaction, fake-authenticated Gmail sync, fake Google/Apple calendar sync, sandbox connector/calendar verification reports, live verification reports, same-second integration verification ID collision coverage, live calendar sync-evidence readiness checks that ignore sandbox approvals, redacted `integrations env-template`, redacted `integrations package latest`, one-step `integrations check --package`, local-only live calendar draft seeding, final completion-audit readiness/package/source-release-audit gating, redacted Google OAuth token format/scope readiness checks, redacted Apple Calendar username/app-password format checks, default-safe live preflight dry-run redaction/source-release-gate coverage, local Google OAuth token writer, audit logs, retention gates, persistent RAG, trust-weighted sparse retrieval ranking, model config, safe model provider adapters, `model status` redacted adapter output, structured output validation, multi-stage workflow metadata, bound portal verification tool execution, email-to-portal deadline fallback, RAG injection filtering, and retrieved-instruction resistance for verified facts and write tools | Re-run before publishing |
...
Current expected result: 184 tests pass.
