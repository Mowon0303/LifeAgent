...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` runs a Python 3.11 package metadata check, 272 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
