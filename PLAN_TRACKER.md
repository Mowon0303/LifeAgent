...
| Reliability tests | Done | 269 unittest cases pass in `sentinel-desk/tests`, covering email/calendar extraction, daily landing workflow, connector redaction in daily summaries, sync summary redaction, relative deadline windows, expanded date forms, adaptive deadline cap/stuffing handling, non-dollar and spelled-out amounts, high/low-confidence and semantic amount false-positive filters, deadline false-positive filters, action false-positive filters, expanded action verbs/noise filters, task review backend, dashboard APIs, retention/audit/approval gates, privacy/release packaging, RAG safety, model/provider boundaries, LangGraph-shaped workflow metadata, tool-verified latest facts, portal fallback citation chaining, CLI `ask`, Gmail-first integration readiness reports and package shape evals, golden-set eval regression gates, and UI contract shape gates | Re-run before publishing |
...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` runs a Python 3.11 package metadata check, 269 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
| Email extraction golden-set eval | Done | `evals/golden/` holds 142 hand-labeled synthetic cases across 10 categories (7 life-admin verticals plus edge_cases, negatives, adversarial) with a written labeling policy in `evals/golden/README.md`; `sentineldesk/evals/email_extract.py` scores field-level precision/recall/F1 on raw and high-confidence layers plus confidence-bucket calibration; `sentineldesk eval email-extract --report-md docs/EVAL_REPORT.md` generates the committed report; measured baseline after relative-deadline, date-form, non-dollar/spelled-out amount, high/low-confidence and semantic amount false-positive filters, deadline false-positive filters, extraction-cap/stuffing handling, action false-positive filters, and action-lexicon support (2026-06-11): raw deadline P=1.000/R=1.000, raw amount P=1.000/R=1.000, raw action P=1.000/R=1.000, high-confidence deadline P=1.000/R=0.574, high-confidence amount P=1.000/R=0.553, and the high bucket still beats or matches the low bucket (deadline 1.000 vs 1.000; amount 1.000 vs 1.000); `tests/test_eval_email_extract.py` gates 9 regression assertions including raised deadline/amount/action metric floors, calibration direction, suppression-injection resistance, and golden-set integrity | Raw extraction queue complete; consider confidence-tier calibration only if product ranking needs it |
...
| Action false-positive filters | Done | Action extraction now suppresses prompt-injected write/calendar/payment instructions, phishing payment actions, noun-like payment schedules, soft phrases such as `pay no attention`, social-notification sign-in bait, optional survey CTAs, pull-request review requests, and corporate all-hands question forms while preserving real sign-in, schedule, and submit actions; targeted tests cover each trap class and preservation cases; golden-set eval removed all 11 raw action false positives, lifting raw action to P=1.000/R=1.000/F1=1.000 | Completed; redacted Gmail-first package shape evals landed later |
...
Current expected result: 269 tests pass.
