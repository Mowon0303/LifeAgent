...
- Next plan to complete: The portfolio engineering loop is closed: eval baseline, UI contract + accepted calendar page, guard-railed model-in-the-loop with cost attribution, stored-evidence ask, pivot postmortem, relative-deadline extraction, non-dollar amount extraction, and action-lexicon expansion are all Done. The next extraction-improvement queue item from `docs/EVAL_REPORT.md` is date-form expansion (`14 July 2026`, `June 5`, ISO datetime `T` suffixes), followed by spelled-out amounts and raw-layer false-positive filters; record the 2-minute portfolio demo when convenient. Calendar live writes remain deferred unless the product workflow needs confirmed external calendar sync.
...
| Action lexicon expansion | Done | `extract_email_facts` now recognizes expanded life-admin action verbs (`contact`, `register`, `apply`, `dispute`, `redeem`, `update`, `cancel`, `verify`, `reply`, `bring`, `report`, `check`, `add`, `print`, `enroll`, `contest`) with targeted filters for email-address local parts, noun-like `update/check/report`, `Terms apply`, app-store update prompts, and conditional security-support prompts; targeted tests cover expanded verbs plus known noise cases; golden-set eval improved raw action recall from 0.805 to 1.000 and raw action F1 from 0.838 to 0.939 | Move to date-form expansion |
...
- Latest eval milestone: the extraction layer now has a quantitative golden-set baseline (`docs/EVAL_REPORT.md`) plus three improvement passes. Relative deadline support removed the `within N days` / `next Friday` / `at least N business days before...` class of false negatives; non-dollar amount support removed the `USD 2,450.00`, `€89.00`, single-decimal `$47.5`, and zero-width `$1,250` misses; action-lexicon expansion removed the remaining explicit CTA false negatives while preserving action precision above the old baseline. Remaining headline gaps are spelled-out amounts, UK/month-day/ISO-datetime date forms, raw-layer false positives from narrative/marketing/injected dates or amounts, and the 10-item extraction cap stuffing attack. These quantified gaps are the work queue for extraction improvements and the motivating evidence for the review-queue product layer.
...
Current expected result: 239 tests pass.
