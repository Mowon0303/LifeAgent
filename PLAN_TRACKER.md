...
- Next plan to complete: The portfolio engineering loop is closed: eval baseline, UI contract + accepted calendar page, guard-railed model-in-the-loop with cost attribution, stored-evidence ask, pivot postmortem, relative-deadline extraction, non-dollar amount extraction, action-lexicon expansion, and date-form expansion are all Done. The next extraction-improvement queue item from `docs/EVAL_REPORT.md` is spelled-out amount extraction (`one thousand two hundred dollars`), followed by raw-layer false-positive filters and extraction-cap/stuffing handling; record the 2-minute portfolio demo when convenient. Calendar live writes remain deferred unless the product workflow needs confirmed external calendar sync.
...
| Email extraction golden-set eval | Done | `evals/golden/` holds 142 hand-labeled synthetic cases across 10 categories (7 life-admin verticals plus edge_cases, negatives, adversarial) with a written labeling policy in `evals/golden/README.md`; `sentineldesk/evals/email_extract.py` scores field-level precision/recall/F1 on raw and high-confidence layers plus confidence-bucket calibration; `sentineldesk eval email-extract --report-md docs/EVAL_REPORT.md` generates the committed report; measured baseline after relative-deadline, date-form, non-dollar amount, and action-lexicon support (2026-06-11): raw deadline P=0.763/R=0.975, raw amount P=0.781/R=0.987, raw action P=0.885/R=1.000, high-confidence deadline P=0.852/R=0.566, high-confidence amount P=0.804/R=0.539, and the risk-word heuristic measurably beats the low bucket (deadline 0.852 vs 0.667; amount 0.804 vs 0.756); `tests/test_eval_email_extract.py` gates 9 regression assertions including raised deadline/amount/action metric floors, calibration direction, suppression-injection resistance, and golden-set integrity | Next extraction work: spelled-out amounts |
...
| Non-dollar amount extraction | Done | `extract_email_facts` now recognizes common currency symbols (`€`, `£`, `¥`, `￥`) and ISO-style prefixes (`USD`, `EUR`, `GBP`, `CNY`, `RMB`), allows one- or two-digit decimal cents (`$47.5`), and removes zero-width separators inside numeric strings before matching; targeted tests cover `USD 2,450.00`, `€89.00`, `$47.5`, and obfuscated `$1,250`; golden-set eval improved raw amount recall from 0.934 to 0.987 and precision from 0.755 to 0.781 | Completed; spelled-out amounts remain a later queue item |
...
- Latest eval milestone: the extraction layer now has a quantitative golden-set baseline (`docs/EVAL_REPORT.md`) plus four improvement passes. Relative deadline support removed the `within N days` / `next Friday` / `at least N business days before...` class of false negatives; non-dollar amount support removed the `USD 2,450.00`, `€89.00`, single-decimal `$47.5`, and zero-width `$1,250` misses; action-lexicon expansion removed the remaining explicit CTA false negatives while preserving action precision above the old baseline; date-form expansion removed the `14 July 2026`, `June 5`, and ISO datetime `T` suffix deadline misses. Remaining headline gaps are spelled-out amounts, raw-layer false positives from narrative/marketing/injected dates or amounts, and the 10-item extraction cap stuffing attack. These quantified gaps are the work queue for extraction improvements and the motivating evidence for the review-queue product layer.
...
Current expected result: 243 tests pass.
