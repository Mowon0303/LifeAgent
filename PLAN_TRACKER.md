...
| Reliability tests | Done | 292 unittest cases pass in `sentinel-desk/tests`, covering email/calendar extraction, stored-evidence reprocessing, daily landing workflow, dashboard daily summary/run APIs, connector redaction in daily summaries, sync summary redaction, relative deadline windows, expanded date forms, adaptive deadline cap/stuffing handling, non-dollar and spelled-out amounts, high/low-confidence and semantic amount false-positive filters, deadline false-positive filters, action false-positive filters including link/HTML action artifacts, expanded action verbs/noise filters, task review grouping/backend, task evidence drill-down, task queue filters/batch navigation, task priority/default sort modes, confirmation-gated bulk task review, local task review history/undo, dashboard APIs, retention/audit/approval gates, privacy/release packaging, RAG safety, model/provider boundaries, LangGraph-shaped workflow metadata, tool-verified latest facts, portal fallback citation chaining, CLI `ask`, Gmail-first integration readiness reports and package shape evals, golden-set eval regression gates, and UI contract shape gates | Re-run before publishing |
| UI contract handoff | Done | `docs/UI_CONTRACT.md` documents response shapes for `/api/calendar/events`, `/api/tasks` with `sort`/`status`/`kind`/`limit` controls and task priority fields, `/api/tasks/evidence`, `/api/tasks/review`, `/api/tasks/review/bulk`, `/api/tasks/review/history`, `/api/tasks/review/undo`, `/api/daily/summary`, `/api/daily/run`, `/api/calendar/sync`, `/api/calendar/drafts/update`, and `/api/ask`, plus the design-token mapping rules (pending=dashed `approval_state: draft`, confirmed=solid `approved`, source-trust captions, undated handling, task review cards with priority reasons, source-evidence drill-down, task queue filters/sort controls, cursor navigation, confirmation-gated bulk review buttons, and review history/undo controls); `fixtures/ui/` holds synthetic sample emails and committed sample responses including `daily_summary.sample.json`; `tests/test_ui_contract.py` gates 35 regression assertions including task filters/sort modes, bulk task review, task review history/undo, task evidence drill-down, daily summary/run, confirm/replay flows, ignore/done review flows, citation payload shape, fixture/live shape sync, and page wiring | Update the contract doc and fixtures together whenever an API shape changes |
...
| Public release privacy audit | Done | `sentineldesk privacy release-audit --path sentinel-desk` detects local ignored development artifacts (`.agent-venv`, `.demo`, `recordings`, `sentineldesk.egg-info`, and `__pycache__/` directories), while `sentineldesk privacy release-package --source sentinel-desk --output /private/tmp/.../sentinel-desk.release.zip` excludes those artifacts; the latest extracted release ZIP passed `privacy release-audit --require-clean` with 121 scanned files and 0 issues | Use `privacy release-package` rather than direct local-tree zipping for public sharing |
...
| GitHub Actions CI | Done | `.github/workflows/ci.yml` uses Node.js 24-compatible `actions/checkout@v6` and `actions/setup-python@v6`, then runs a Python 3.11 package metadata check, 292 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit; the local CI-equivalent command set passed | Check the first remote Actions run after pushing to GitHub |
...
- Current landing focus: the calendar assistant now calls the daily-loop APIs and full task review queue so the user sees inbox/stored-mail status, grouped task queue, priority/date/recent sort controls, kind/status filters, previous/current/next task navigation, amount/action task cards, source-evidence drill-down, calendar draft summary, connector readiness, uncertainty, and confirmation-gated actions in one surface.
...
Current expected result: 292 tests pass.
