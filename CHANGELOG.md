...
- Added GitHub Actions CI at `.github/workflows/ci.yml`: Python 3.11 package metadata check, 283 unittests, `compileall`, golden email extraction eval JSON, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit.
...
- Added `tests/test_ui_contract.py` with 31 regression gates: calendar item, task, task filter, bulk task review, task evidence drill-down, daily summary, and ask field shapes; confirm flow turning `draft` into `approved`; confirmation-ID replay blocking; ignore/done review flows; audited daily run; fixture-vs-live shape sync; synthetic-fixture guard; and `/calendar` page wiring including a no-external-scripts assertion.
