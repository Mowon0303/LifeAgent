...
- Removed GitHub repository description/topics copy from `sentinel-desk/docs/CASE_STUDY.md` and kept repository metadata out of the active project plan after user direction.
- Calibrated retained deadline and amount facts into the high-confidence layer after false-positive filters run; the 144-case golden eval now reports high-confidence deadline and amount P=1.000/R=1.000/F1=1.000 while action confidence remains flat by design.
- Archived `sentinel-desk/docs/DEMO_VIDEO_SCRIPT.md` and `sentinel-desk/docs/RECORDING_CHECKLIST.md` as cancelled recording artifacts, removed the demo script from the root active-doc list, and shifted interview/project docs to first-run acceptance and product-landing proof.
...
- Connected the calendar assistant UI to the daily landing loop: `/api/daily/summary` exposes a read-only local snapshot for page load, `/api/daily/run` writes an audited local daily check, and the assistant panel now shows stored mail, grouped review queue, local calendar drafts, connector readiness, and external-write status.
- Promoted task review into the calendar assistant UI: the page now reads the full `/api/tasks` queue and renders amount/action task cards with local-only `done`, `needs_verification`, `reviewed`, and `ignored` controls backed by audited `/api/tasks/review` calls.
- Added source-evidence drill-down for task cards: `/api/tasks/evidence` returns local email metadata, matched facts, and bounded body previews without external reads or writes, and the assistant exposes it through `查看证据`.
- Added task queue filters and cursor navigation to the calendar assistant: `/api/tasks` now supports `kind`, `status`, and `limit` filters; `sentineldesk tasks list` mirrors `--kind`; and the assistant panel can review filtered amount/action/deadline queues with previous/current/next navigation instead of cycling one unfiltered card at a time.
- Added confirmation-gated bulk task review for the current filtered queue: `/api/tasks/review/bulk` and `sentineldesk tasks bulk-review` require explicit confirmation IDs, write only local review/audit/approval records, block confirmation-ID replay, and the assistant exposes bulk done/reviewed/ignored/needs-verification buttons for the filtered queue.
- Added local task review history and confirmation-gated undo: `/api/tasks/review/history`, `/api/tasks/review/undo`, `sentineldesk tasks history`, and `sentineldesk tasks undo` recover recent single/bulk review mistakes through local review/audit/approval records only, with no Gmail refresh or external calendar write.
...
- Added a local-only Gmail first-run readiness checklist: `sentineldesk integrations gmail-readiness`, `/api/gmail/readiness`, `/api/daily/summary.gmail_readiness`, and the calendar assistant's `gmailReadiness` panel show OAuth env shape, optional dependency availability, stored cursor/local evidence, and the safe next command without calling Gmail or writing external systems.
- Added redacted Gmail sync failure diagnostics: failed `email sync-gmail` and `daily run --sync-gmail` attempts now write local `email.connector.sync.failed` audit rows, return classified recovery guidance, and feed `sentineldesk integrations gmail-sync-diagnostics`, `/api/gmail/sync-diagnostics`, `/api/daily/summary.gmail_sync_diagnostics`, and the assistant's `gmailSyncDiagnostics` panel without exposing raw exception text, account IDs, cursors, tokens, or query text.
- Added an explicit assistant-triggered Gmail readonly sync/retry path: shared sync helper, `POST /api/gmail/sync?confirm=1`, no-network confirmation-required preview, redacted success/failure responses, and a `同步 Gmail（外部只读）` UI button labeled as an external read with local-only writes.
- Added one-command first-run MVP acceptance: `sentineldesk acceptance first-run` prepares the synthetic local product state, verifies email ingest, task review, calendar draft visibility, tool-first cited ask behavior, Gmail readiness/diagnostics, UI wiring, audit logging, and no external network/write side effects; CI now runs this gate.
...
- Added `sentineldesk email reprocess` and `daily run --reprocess-stored` so extractor fixes can be applied to already persisted local email evidence without another Gmail call; the command rewrites local facts, optionally upserts local calendar drafts, reports old/new fact counts, and writes an `email.reprocess` audit event with `external_writes_performed: false`.
...
- Added `sentinel-desk/docs/CASE_STUDY.md`, a portfolio-ready write-up covering the problem, email-first product decision, user flow, architecture, agent boundaries, safety model, evidence table, and demo scenario.
- Added GitHub Actions CI at `.github/workflows/ci.yml`: Python 3.11 package metadata check, 309 unittests, `compileall`, golden email extraction eval JSON, first-run MVP acceptance, direct email-first product-state prep, `daily run`, redacted-output privacy audit, clean source release packaging, and extracted release audit.
...
- Added absolute deadline false-positive filters for prompt-injected/social-engineering dates, phishing payment dates, marketing/promo dates, narrative or past event dates, replaced old due dates, forwarded `Date:` headers, optional/informational event dates, and account-history stuffing dates; targeted tests preserve real due-date, grace-period, scheduled service, and final-balance deadlines.
- Added adaptive deadline cap handling: ordinary date piles still stop at 10 extracted deadlines, while explicit lease/payment/premium/billing schedule contexts can return up to 20 dates; targeted tests cover both a 12-date lease payment schedule and an unstructured stuffing pile.
- Added action false-positive filters for prompt-injected write/calendar/payment instructions, phishing payment actions, noun-like payment schedules, soft phrases such as `pay no attention`, social-notification sign-in bait, optional survey CTAs, pull-request review requests, and corporate all-hands question forms; targeted tests preserve real sign-in, schedule, and submit actions.
...
- Added `fixtures/ui/` with synthetic sample emails and committed sample API responses (`calendar_events.sample.json`, `tasks.sample.json`, `daily_summary.sample.json`, `ask_answer.sample.json`).
- Added `tests/test_ui_contract.py` with 43 regression gates: calendar item, task, task filter/sort/view, bulk task review, task review history/undo, task evidence drill-down, daily summary, Gmail readiness, Gmail sync diagnostics, Gmail sync confirmation/redaction/failure behavior, and ask field shapes; confirm flow turning `draft` into `approved`; confirmation-ID replay blocking; ignore/done review flows; audited daily run; fixture-vs-live shape sync; synthetic-fixture guard; and `/calendar` page wiring including a no-external-scripts assertion.
- Added proper static content types (`.css`, `.js`, `.json`, `.svg`) to the dashboard server.

- Added `sentinel-desk/evals/golden/` with 144 hand-labeled synthetic email cases across 10 categories (lease/rent, billing/utility, bank/card, immigration/school, subscriptions, insurance/medical, tax/government, edge cases, negatives, adversarial), including marketing/receipt/injection false-positive traps, live-run-derived link/HTML action traps, relative-deadline and non-dollar-currency recall gaps, attachment-only and subject-only facts, zero-width obfuscation, and a 10-date stuffing attack against the extraction cap.
...
- Added `tests/test_eval_email_extract.py` with 9 regression gates: golden-set integrity (size, categories, unique IDs, `.example` senders), raw and high-confidence metric floors set just below the measured baseline, retained deadline/amount high-bucket calibration, action flat-confidence structure, suppression-injection resistance (`adv-010` real facts must survive), and negative-category purity.
...
- Fixed all deadline false positives in the current golden set: raw deadline false positives fell from 37 to 0 and high-confidence deadline false positives fell from 12 to 0 while keeping raw deadline recall above 0.98.
- Fixed the final 2 raw deadline false negatives in the current golden set by preserving tail dates in structured monthly lease payment schedules; raw deadline is now P=1.000/R=1.000/F1=1.000.
- Fixed all 11 raw action false positives in the current golden set while keeping action recall at 1.000; raw deadline, raw amount, and raw action are now all P=1.000/R=1.000/F1=1.000.
- Fixed committed UI calendar/task sample fixtures so the newsletter negative sample no longer appears as a draft deadline after deadline false-positive filtering.
...
- Raised the email-extraction eval gates after deadline false-positive filtering: raw deadline floors are now P>=0.99/R>=0.98 and high-confidence deadline floors are now P>=0.99/R>=0.57.
- Raised the email-extraction eval gates after extraction-cap/stuffing handling: raw deadline floors are now P>=0.99/R>=0.99.
- Raised the email-extraction eval gates after action false-positive filtering: raw action floors are now P>=0.99/R>=0.99.
...
- `cd sentinel-desk && python3 -B -m sentineldesk --home /private/tmp/lifeagent-action-fp-release-home privacy release-package --source . --output /private/tmp/lifeagent-action-fp-20260611.release.zip` wrote a 118-file source release ZIP excluding 10 local runtime artifacts, and `privacy release-audit --require-clean` passed on the extracted package with 0 issues.
- `cd sentinel-desk && python3 -B -m unittest discover -s tests -q` passed with 261 tests after adding adaptive deadline cap/stuffing handling and raised raw deadline recall gates.
- `cd sentinel-desk && python3 -B -m sentineldesk eval email-extract --golden evals/golden --report-md docs/EVAL_REPORT.md` measured the extraction-cap improvement: raw deadline P=1.000/R=1.000/F1=1.000 (tp=122/fp=0/fn=0), raw amount P=1.000/R=1.000/F1=1.000, and high-confidence deadline remains P=1.000/R=0.574/F1=0.729; no deadline or amount false positives/false negatives remain in the current golden set.
- `cd sentinel-desk && python3 -m compileall -q sentineldesk tests` passed after the adaptive deadline cap update.
- `cd sentinel-desk && python3 -B -m sentineldesk --home /private/tmp/lifeagent-deadline-cap-release-home privacy release-package --source . --output /private/tmp/lifeagent-deadline-cap-20260611.release.zip` wrote a 118-file source release ZIP excluding 10 local runtime artifacts, and `privacy release-audit --require-clean` passed on the extracted package with 0 issues.
- `cd sentinel-desk && python3 -B -m unittest discover -s tests -q` passed with 259 tests after adding deadline false-positive filters and raised deadline precision gates.
- `cd sentinel-desk && python3 -B -m sentineldesk eval email-extract --golden evals/golden --report-md docs/EVAL_REPORT.md` measured the deadline false-positive improvement: raw deadline P=1.000/R=0.984/F1=0.992 (tp=120/fp=0/fn=2) and high-confidence deadline P=1.000/R=0.574/F1=0.729 (tp=70/fp=0/fn=52); raw deadline false positives fell from 37 to 0 and high-confidence deadline false positives fell from 12 to 0. The remaining raw deadline false negatives are extraction-cap/tail-schedule dates in `edge-005`.
- `cd sentinel-desk && python3 -m compileall -q sentineldesk tests` passed after the deadline false-positive filter update.
- `cd sentinel-desk && python3 -B -m sentineldesk --home /private/tmp/lifeagent-deadline-fp-release-home privacy release-package --source . --output /private/tmp/lifeagent-deadline-fp-filters-20260611.release.zip` wrote a 118-file source release ZIP excluding 10 local runtime artifacts, and `privacy release-audit --require-clean` passed on the extracted package with 0 issues.
