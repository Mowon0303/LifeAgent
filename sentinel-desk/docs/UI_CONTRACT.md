...
Read-only daily landing summary for the assistant panel. It returns stored email counts, fact counts, grouped task queue counts and optional queue rows, local calendar draft counts and optional calendar items, redacted connector readiness, a Gmail first-run readiness checklist, local Gmail sync diagnostics, a local review receipt, safety flags, and safe next actions. This endpoint does **not** write a `daily.run` audit event; it is safe for page load and refresh polling.

Top-level fields: `status`, `generated_at`, `mode`, `sync`, `email`, `tasks`, `calendar`, `connectors`, `gmail_readiness`, `gmail_sync_diagnostics`, `review_receipt`, `safety`, `next_actions`.
...
Read-only local diagnostic summary over Gmail sync audit rows. It returns `status` (`no_attempt`, `ready`, or `failed`), redacted `account_id`, the latest redacted failure and success summaries, recent failure/success counts, a safe `next_action`, `external_network: false`, and `external_writes_performed: false`.

Failure rows include only classified metadata: `category`, `error_type`, safe `detail`, command label, query presence/length, cursor presence, env var names, and side-effect flags. The endpoint never returns raw exception text, OAuth values, provider tokens, real account IDs, connector cursors, or query text. Failed CLI Gmail sync attempts write `email.connector.sync.failed` audit rows so this endpoint can explain what to fix after OAuth/first-sync attempts.
